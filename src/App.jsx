    ...
    return (
      ...
      {authScreen === 'login' && (
        <AuthForm type="login" email={authEmail} pass={authPass} error={authError}
          ...
          onGoogleLogin={handleGoogleLogin} isSupabase={isSupabaseReady()} />
      )}
      {authScreen === 'signup' && (
...
function AuthForm({ type, email, pass, name, error, onEmail, onPass, onName, onSubmit, onBack, onGoogleLogin, isSupabase }) {
  return (
          ...
          {isSupabase && (
            <>
              <div style={{display:'flex',alignItems:'center',gap:12,margin:'16px 0'}}>
                ...
                <span style={{fontSize:12,color:'var(--text-mut)'}}>o</span>
                <div style={{flex:1,height:1,background:'var(--border)'}} />
              </div>
              <button onClick={onGoogleLogin}
                style={{width:'100%',padding:'12px',fontSize:14,fontWeight:600,background:'var(--bg-secondary)',border:'1px solid var(--border)',borderRadius:'var(--radius-md)',color:'var(--text-pri)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
